...
public class CS400Graph<NodeType,EdgeType extends Number> implements GraphADT<NodeType,EdgeType> {
    ...
    protected class Path implements Comparable<Path> {
        ...
        public Path(Path copyPath, Edge extendBy) {
            ...
            this.distance = copyPath.distance + extendBy.weight.doubleValue(); //Works?
            this.dataSequence = new LinkedList<>(); //Deep Copy
            for(NodeType i : copyPath.dataSequence){
    ...
    protected Path dijkstrasShortestPath(NodeType start, NodeType end) {
        if(vertices.get(start) == null || vertices.get(end) == null){ //Sanity Check
            throw new NoSuchElementException();
        }

        PriorityQueue<Path> discoveredPaths = new PriorityQueue<Path>();
        Hashtable<Vertex, Path> bestPaths = new Hashtable<>();
        Hashtable<Vertex, Boolean> visitedNodes = new Hashtable<>(); //input a node and we know if visited or not.

        Path starterPath = new Path(vertices.get(start));
        discoveredPaths.add(starterPath);
        
        while(!discoveredPaths.isEmpty()){
            Path currentPath = discoveredPaths.remove(); //removes head (shortest path of all paths in Q)
            Boolean visited = visitedNodes.getOrDefault(currentPath.end, false); //first path found to endpoint is thes hortest.

            if(!visited){ //if we don't have a shortest path to the endpoint of smallestPath. Do the shit
                visitedNodes.put(currentPath.end, true);
                bestPaths.put(currentPath.end, currentPath);

                /*
                for each unvisited node adjacent to our currentPath.end. create extended paths and insert into priority queue.
                vertexes contain a linkedList of edges leaving. (edges)
                make a new path for each leaving edge.
                */
                for(Edge outBoundEdge : currentPath.end.edgesLeaving){

                    if(!visitedNodes.getOrDefault(outBoundEdge.target, false)){
                        Path extendPath = new Path(currentPath, outBoundEdge);
                        discoveredPaths.add(extendPath);
                    }
                }
            }  
        }
        if(visitedNodes.getOrDefault(vertices.get(end), null) == null){ //Sanity Check
            throw new NoSuchElementException();
        }
        
        Path shortestPath = bestPaths.getOrDefault(vertices.get(end), null);
        if(shortestPath == null){ //if no best path found. Throw
            throw new NoSuchElementException();
        }
        return shortestPath; 
    }
    ...
    If your search fails to find any path from the start to end vertices, 
    then this method should throw a NoSuchElementException to indicate that this has happened. 
    */
