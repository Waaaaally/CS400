...
public class BookMapperBackend implements IBookMapperBackend {
    ...
    //protected BDIterableMapADT HashtableTitle = new BDIterableMapADT();
    //protected BDIterableMapADT HashtableAuthors = new BDIterableMapADT();
    protected BDIterableMapADT HashtableISBN = new BDIterableMapADT();
    //protected IteratorHashTable HashtableISBN = new IteratorHashTable();


    private String setAuthor = null;
    //public BDIterableMapADT booksTable = new BDIterableMapADT();
    @Override
    /**
    ...
    public void addBook(IBook book) {
        ...
        HashtableISBN.put(book.getISBN13(),book);
    }
    ...
    public List<IBook> searchByTitleWord(String word) {
        ...
        Iterator<IBook> iterable = HashtableISBN.iterator();
        while (iterable.hasNext()) {
            IBook currentBook =  iterable.next();
            if (currentBook.getTitle().contains(word)) {
                if (this.getAuthorFilter() != null) {
                    if (currentBook.getAuthors().contains(getAuthorFilter())) {
    ...
    public IBook getByISBN(String ISBN) {
        // TODO Auto-generated method stub
        if (HashtableISBN.containsKey(ISBN)) {
	    return (IBook) HashtableISBN.get(ISBN);
        }
        else {
            return null;
        } 
